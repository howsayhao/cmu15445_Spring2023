...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower" || func_name == "upper") {
    if (args.size() != 1) {
      throw Exception(fmt::format("func call {} supported, but in wrong parameters number", func_name));
    }
    if (func_name == "lower") {
      return std::make_shared<StringExpression>(StringExpression(args.back(), StringExpressionType::Lower));
    }
    return std::make_shared<StringExpression>(StringExpression(args.back(), StringExpressionType::Upper));
  }
  throw Exception(fmt::format("func call {} not supported in planner yet", func_name));
}
