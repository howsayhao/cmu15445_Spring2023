  ...
  if (free_list_.empty()) {  // 没有空的frame了，需要找Replacer腾出空间
    if (replacer_->Evict(&frame_to_set)) {  // 若成功则说明是处于UnPin的状态，即evictable // 只有各个线程对该page/frame
      ...
      if (pages_[frame_to_set].IsDirty()) {
        // auto getpageid = pages_[frame_to_set].GetPageId();
        // auto data = pages_[frame_to_set].GetData();
        // latch_.unlock();
        // rwdlatch_.WLock();
        // disk_manager_->WritePage(getpageid, data);
...
auto BufferPoolManager::FetchPage(page_id_t page_id, [[maybe_unused]] AccessType access_type) -> Page * {
  ...
  if (pages_[frame_to_fetch].IsDirty()) {
    // auto getpageid = pages_[frame_to_fetch].GetPageId();
    // auto data = pages_[frame_to_fetch].GetData();
    // latch_.unlock();
    // rwdlatch_.WLock();
    // disk_manager_->WritePage(getpageid, data);
  ...
  // latch_.unlock();
  // rwdlatch_.RLock();
  // disk_manager_->ReadPage(page_id, data);  // dirty不按规矩来
  // rwdlatch_.RUnlock();
  // latch_.lock();
...
auto BufferPoolManager::FlushPage(page_id_t page_id) -> bool {
  ...
  // auto data = pages_[frame_to_flush].GetData();
  // latch_.unlock();
  // rwdlatch_.WLock();
...
auto BufferPoolManager::DeletePage(page_id_t page_id) -> bool {
  ...
  if (pages_[frame_to_delete].IsDirty()) {
    // auto getpageid = pages_[frame_to_delete].GetPageId();
    // auto data = pages_[frame_to_delete].GetData();
    // latch_.unlock();
    // rwdlatch_.WLock();
    // disk_manager_->WritePage(getpageid,
    //  data);  // dirty不按规矩来
